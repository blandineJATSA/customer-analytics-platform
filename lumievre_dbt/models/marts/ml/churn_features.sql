...
with campaign_obs as (
    ...
    from {{ ref('stg_marketing__campaign_sends') }}
    where sent_date < timestamp({{ cutoff }})
...
select
    ...
    churn.churned
from {{ ref('int_customer_rfm') }} rfm
inner join {{ ref('int_churn_labels') }} churn on rfm.customer_id = churn.customer_id
left join {{ ref('customer_360') }} c on rfm.customer_id = c.customer_id
