...
left join {{ ref('stg_marketing__preferences') }} mkt on crm_dedup.customer_id = mkt.customer_id
left join {{ ref('stg_loyalty__program') }} loy on crm_dedup.customer_id = loy.customer_id
left join support_agg sup on crm_dedup.customer_id = sup.customer_id
left join {{ ref('int_churn_labels') }} churn on crm_dedup.customer_id = churn.customer_id
left join {{ ref('int_customer_rfm') }} rfm on crm_dedup.customer_id = rfm.customer_id