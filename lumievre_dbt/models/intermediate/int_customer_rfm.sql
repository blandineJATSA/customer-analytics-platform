-- Reproduit le RFM validé en Phase 1 : calculé uniquement sur la fenêtre
...
with observation as (
    ...
    where invoice_date < timestamp({{ cutoff }})
),

rfm_raw as (
    select
        customer_id,
        date_diff(date({{ cutoff }}), date(max(invoice_date)), day) as recency,
...
loyal_only as (
    select r.*
    from rfm_raw r
    inner join {{ ref('int_churn_labels') }} c
